...
TEST_F(HIRTypeTest, Parse) {
  ...
  EXPECT_EQ(Type::parse("Banana"), TBottom);
  EXPECT_EQ(Type::parse("Long[123]"), TBottom);
}
...
TEST_F(HIRTypeTest, ReflowSimpleTypes) {
  ...
  b2->append<MakeListTuple>(false, v1, 0, FrameState{});
  b2->append<Branch>(b3);
  ...
  EXPECT_EQ(v1->type(), TList);
  EXPECT_EQ(v2->type(), TDict | TList);
}
...
TEST_F(HIRTypeTest, ReflowLoopTypes) {
  ...
  b0->append<MakeListTuple>(true, v0, 4, FrameState{});
  b0->append<Branch>(b1);
  ...
  EXPECT_EQ(v0->type(), TTuple);
  EXPECT_EQ(v1->type(), TTuple | TDict);
