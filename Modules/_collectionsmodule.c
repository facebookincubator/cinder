...
static PyObject *
_collections_new_tuple(PyObject *module, PyObject **stack, Py_ssize_t nargs)
{
    if (nargs == 0) {
        PyErr_SetString(PyExc_TypeError, "_new_tuple expects at least one argument");
        return NULL;
    }
    if (_Py_UNLIKELY(!(PyType_Check(stack[0]) && PyType_IsSubtype((PyTypeObject *)stack[0], &PyTuple_Type)))) {
        PyErr_SetString(PyExc_TypeError, "1st argument to _new_tuple should be type");
        return NULL;
    }
    PyTypeObject *t = (PyTypeObject *)stack[0];
    PyObject *res = t->tp_alloc(t, nargs - 1);
    if (res == NULL) {
        return NULL;
    }
...
static struct PyMethodDef module_functions[] = {
    ...
    {"_new_tuple", (PyCFunction)_collections_new_tuple, METH_FASTCALL, "creates/populates new subclass of tuple"},
    {NULL,       NULL}          /* sentinel */
};
