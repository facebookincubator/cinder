...
static int
future_cancel_impl(FutureObj *fut, PyObject *Py_UNUSED(ignored));

static int
task_cancel_impl(TaskObj *self, PyObject *Py_UNUSED(ignored));
...
static int GatheringFuture_cancel_impl(_GatheringFutureObj *self, PyObject *Py_UNUSED(ignored));
...
static int
FutureLike_cancel(PyObject *fut) {
    PyObject *res = call_method_id_noarg(fut, cancel_name);
    if (res == NULL) {
        return -1;
    }
    int is_true = PyObject_IsTrue(res);
...
populate_method_table(PyMethodTableRef *tableref, PyTypeObject *type) {
    ...
    else if (is_impl_method(cancel, (PyCFunction)_asyncio__GatheringFuture_cancel_impl)) {
...
prepare_args(PyObject *func, PyObject *arg) {
    ...
    if (args == NULL) {
        if ((args = _PyTuple_NewNoTrack(nargs)) == NULL) {
            return NULL;
        }
    }
...
static int
_future_raise_cancelled_error(FutureObj *fut)
{
  PyObject *err;
  err = _PyObject_CallNoArg(asyncio_CancelledError);
  if (!err) {
    return -1;
  }
  PyErr_SetObject(asyncio_CancelledError, err);
  Py_DECREF(err);
  return 0;
}
...
future_get_result(FutureObj *fut, PyObject **result)
{
    if (fut->fut_state == STATE_CANCELLED) {
      _future_raise_cancelled_error(fut);
      return -1;
    }
...
{
    ENSURE_FUTURE_ALIVE(self)
    int ok = future_cancel_impl(self, NULL);
    if (ok < 0) {
        return 0;
    }
    if (ok) {
...
static PySendResult
FutureIter_itersend(PyThreadState* Py_UNUSED(tstate),
                   futureiterobject *it,
...
FutureIter_iternext(futureiterobject *it)
{
    ...
    PySendResult gen_status = FutureIter_itersend(NULL, it, NULL, &result);
...
future_new_iter(PyObject *fut)
{
    ...
    if (fi_freelist_len) {
        ...
        it = PyObject_GC_New(futureiterobject, (PyTypeObject *)&FutureIterType);
        if (it == NULL) {
            return NULL;
        }
    }
...
task_call_step(TaskObj *task, PyObject *arg)
{
    if (Task_CheckExact(task)) {
        ...
        return tableref->step_thunk((PyObject*)task, arg, tableref->step);
    }
}
...
static PyObject *
wakeup_meth_call(TaskObj *task, PyObject *fut)
{
    return task_wakeup(task, fut);
}

PyMethodDef _TaskWakeupMethod = {
    "task_wakeup_method",
    (PyCFunction)wakeup_meth_call,
    METH_O,
    NULL
};
...
_is_coro_suspended(PyObject *coro)
{
    if ((PyCoro_CheckExact(coro) || PyGen_CheckExact(coro)) &&
        _PyGen_IsSuspended((PyGenObject *)coro)) {
...
/*[clinic end generated code: output=11f9b20749ccca5d input=497f80bc9ce726b5]*/
{
    ...
    if (loop != Py_None) {
        ...
        PyObject *current_loop = get_event_loop();
...
task_cancel_impl(TaskObj *self, PyObject *Py_UNUSED(ignored)) {
    ...
    if (self->task_fut_waiter) {
        ...
        is_true = tableref->cancel(self->task_fut_waiter, NULL);
...
{
    int ok = task_cancel_impl(self, NULL);
    if (ok == -1) {
        return NULL;
    }
    if (ok) {
...
task_set_fut_waiter_impl(TaskObj *task, PyObject *result)
{
    ...
    if (Future_CheckExact(result) || Task_CheckExact(result)) {
        ...
        wrapper = PyCFunction_New(&_TaskWakeupMethod, (PyObject*)task);
        if (wrapper == NULL) {
            goto fail;
        }
        ...
        if (task->task_must_cancel) {
            int is_true = Future_CheckExact(result)
                ? future_cancel_impl((FutureObj*)result, NULL)
                : task_cancel_impl((TaskObj *)result, NULL);
    ...
    if (blocking_state == BLOCKING_TRUE || blocking_state == BLOCKING_FALSE) {
        ...
        if (task->task_must_cancel) {
            int is_true = result_tableref->cancel(result, NULL);
    ...
    int res = PyObject_IsInstance(result, (PyObject*)&PyGen_Type);
    if (res < 0) {
        goto fail;
    }
    if (res) {
...
task_step_impl(TaskObj *task, PyObject *exc)
{
    ...
    if (gen_status == PYGEN_RETURN || gen_status == PYGEN_ERROR) {
        ...
        if (PyErr_ExceptionMatches(asyncio_CancelledError)) {
          ...
          PyErr_Clear();
          int ok;
          ok = future_cancel_impl((FutureObj*)task, NULL);
          if (ok < 0) {
            return NULL;
          }
          if (ok) {
...
static PyObject *
_asyncio_ContextAwareTask_base_step(TaskObj *self, PyObject *exc)
{
#ifdef EXTENDED_USDTS
    // Ideally, we would gate this with a semaphore based probe, but bpftrace
    // has some issues with high memory usage when using those (these are being
    // fixed in https://github.com/iovisor/bpftrace/pull/1374).
    // TODO (aniketpanse): Consider using semaphore based tracing probes

    _Py_IDENTIFIER(view_name);
    _Py_IDENTIFIER(getPythonContext);

    int res;
    PyObject *view_name, *py_ctx, *folly_ctx;
    folly_ctx = ((ContextAwareTaskObj *)self)->context;

    if (folly_ctx != NULL) {
        // Get a reference to the python context, which has the view names
        py_ctx =
            _PyObject_CallMethodId(folly_ctx, &PyId_getPythonContext, NULL);
        if (!PyErr_Occurred()) {
            // Get the view name
            res = _PyObject_LookupAttrId(py_ctx, &PyId_view_name, &view_name);
            if (res != -1 && view_name != Py_None && view_name != NULL) {
                //
                FOLLY_SDT(python,
                          asyncio_contextawaretask_setctx,
                          PyUnicode_DATA(view_name),
                          py_ctx);
            } else {
                FOLLY_SDT(python, asyncio_contextawaretask_unsetctx);
            }
            Py_XDECREF(view_name);
        } else {
            // no py context, move on...
            PyErr_Clear();
        }
    }
#endif
    return _asyncio_Task__step_impl(self, exc);
}
...
// clang-format on
{
    ...
    return hooks->execute_step((PyObject *)self,
                               exc,
                               self->context,
                               (execute_base_step)_asyncio_ContextAwareTask_base_step);
}
...
AsyncLazyValue_set_result(AsyncLazyValueObj *self, PyObject *res)
{
    ...
    if (notify_futures(self->alv_futures, future_set_result, res) < 0) {
        return -1;
    }
...
AsyncLazyValue_set_error(AsyncLazyValueObj *self, PyObject *exc)
{
    ...
    if (PyObject_IsInstance(exc, asyncio_CancelledError)) {
        ...
        ok = notify_futures(self->alv_futures, future_set_exception, exc);
    }
...
AsyncLazyValue_ensure_future(AsyncLazyValueObj *self, PyObject *loop)
{
    ...
    case ALV_DONE: {
        ...
        int ok = future_set_result((FutureObj *)fut, self->alv_result);
        if (ok < 0) {
            Py_DECREF(fut);
            return NULL;
        }
...
static PySendResult
AsyncLazyValue_itersend(PyThreadState *Py_UNUSED(tstate),
...
{
    PyObject *result;
    PySendResult gen_status = AsyncLazyValue_itersend(NULL, self, NULL, &result);
    return gen_status_to_iter(gen_status, result);
}
                ...
                PyObject *kwargs)
{
    if (PyMethod_Check(func)) {
        ...
        return _PyObject_Call_Prepend_FastCallDict(
            meth_func, self, args, nargs | _Py_AWAITED_CALL_MARKER, kwargs);
    }
                                             ...
                                             int *did_step)
{
    ...
    *did_step = 1;
    if (((PyWaitHandleObject *)result)->wh_waiter == NULL) {
        PyObject *retval = ((PyWaitHandleObject *)result)->wh_coro_or_result;
...
/**
  ...
  */
static PySendResult
AsyncLazyValueCompute_itersend(PyThreadState *tstate,
...
{
    PyObject *res;
    PySendResult gen_status = AsyncLazyValueCompute_itersend(PyThreadState_GET(), self, val, &res);
...
AsyncLazyValueCompute_close_(AsyncLazyValueComputeObj *self)
{
    ...
    // close subgenerator
    int err = _PyGen_close_yf(self->alvc_coroobj);
    Py_CLEAR(self->alvc_coroobj);
...
{
    if (self->alvc_coroobj == NULL) {
        _PyGen_restore_error(type, val, tb);
        return NULL;
    }
    ...
    if (PyErr_GivenExceptionMatches(type, PyExc_GeneratorExit)) {
        ...
        if (_PyGen_restore_error(type, val, tb) == -1) {
            return NULL;
        }
        return AsyncLazyValueCompute_handle_error(
            self, PyThreadState_GET(), 1, 0);
...
static inline int
_cancel_futures(_GatheringFutureObj *gfut)
{
    int ret = 0;
    ...
    FOREACH_INDEX(gfut->gf_datamap, i)
    {
        ...
        int is_true = t->cancel(child, NULL);
        if (is_true < 0) {
            return -1;
        }
        if (is_true) {
...
static int
GatheringFuture_cancel_impl(_GatheringFutureObj *self, PyObject *Py_UNUSED(ignored))
{
    if (future_done((FutureObj *)self)) {
        return 0;
    }
    int ret = _cancel_futures(self);
    if (ret < 0) {
        return -1;
    }
    if (ret) {
...
static PyObject *
_asyncio__GatheringFuture_cancel_impl(_GatheringFutureObj *self)
/*[clinic end generated code: output=c3be9bd8f57fb631 input=fcdd2dad96c0022c]*/
{
    int is_true = GatheringFuture_cancel_impl(self, NULL);
    if (is_true < 0) {
        return NULL;
    }
    if (is_true) {
...
// clang-format off
static PyMethodDef _GatherinFutureType_methods[] = {
    _ASYNCIO_FUTURE_RESULT_METHODDEF
    _ASYNCIO_FUTURE_EXCEPTION_METHODDEF
    _ASYNCIO_FUTURE_SET_RESULT_METHODDEF
    _ASYNCIO_FUTURE_SET_EXCEPTION_METHODDEF
...
static PyTypeObject _GatheringFutureType = {
    ...
    .tp_iter = (getiterfunc)future_new_iter,
    .tp_methods = _GatherinFutureType_methods,
    .tp_getset = FutureType_getsetlist,
...
_wrap_result_value(PyObject *result, PyObject *loop, int awaited)
{
    if (awaited) {
        Py_INCREF(result);
        return _PyWaitHandle_New(result, NULL);
    }
                 ...
                 int awaited)
{
    ...
    for (Py_ssize_t i = 0; i < nitems; ++i) {
        ...
        // need to wait until coroutinw is resumed - create done_callback if it
...
{
    return _gather_worker(args, PyVectorcall_NARGS(nargsf), Py_None, 0, 1, _Py_AWAITED_CALL(nargsf) != 0);
}
                                 ...
                                 int return_exceptions)
{
    ...
    PyObject *result =
        _gather_worker(items, nitems, Py_None, return_exceptions, 1, _Py_AWAITED_CALL(nargsf) != 0);
    if (iterable != args[0]) {
        Py_DECREF(iterable);
    }
    return result;
...
_asyncio_ig_gather_no_raise(PyObject *module,
                            ...
                            size_t nargsf)
{
    return _gather_worker(args, PyVectorcall_NARGS(nargsf), Py_None, 1, 1, _Py_AWAITED_CALL(nargsf) != 0);
}
...
PyInit__asyncio(void)
{
    ...
    if (PyType_Ready((PyTypeObject *)&FutureIterType) < 0) {
        return NULL;
    }
    ...
    if (PyType_Ready((PyTypeObject *)&_AsyncLazyValue_Type) < 0) {
        return NULL;
    }
    if (PyType_Ready((PyTypeObject *)&_AsyncLazyValueCompute_Type) < 0) {
        return NULL;
    }
    ...
    Future_TableRef = create_method_table(&FutureType);
    if (Future_TableRef == NULL) {
        return NULL;
    }
    GatheringFuture_TableRef =
        create_method_table(&_GatheringFutureType);
