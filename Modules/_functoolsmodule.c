...
partial_call(partialobject *pto, PyObject *args, PyObject *kwargs)
{
    ...
    /* Note: PySequence_Concat() is optimized for empty tuples */
...
static PyObject *
keyobject_vectorcall(keyobject *ko, PyObject *const *stack, size_t nargs, PyObject *kwnames);
...
static PyTypeObject keyobject_type = {
    ...
    offsetof(keyobject, vectorcall),    /* tp_vectorcall_offset */
    0,                                  /* tp_getattr */
    0,                                  /* tp_setattr */
    0,                                  /* tp_as_async */
    0,                                  /* tp_repr */
...
keyobject_vectorcall(keyobject *ko, PyObject *const *args, size_t nargsf, PyObject *kwnames)
{
    ...
    keyobject *result = PyObject_New(keyobject, &keyobject_type);
    if (!result) {
        return NULL;
    }
    Py_INCREF(ko->cmp);
...
static PyObject *
uncached_lru_cache_wrapper(lru_cache_object *self,
                           PyObject *const *args,
                           ssize_t nargsf,
                           PyObject *kwnames)
{
    self->misses++;
    return _PyObject_Vectorcall(self->func, args, nargsf, kwnames);
}

static inline Py_ssize_t
compute_key_size(Py_ssize_t nargs, PyObject *kwnames, int typed)
{
    ...
    if (kwnames && (kw_count = PyTuple_GET_SIZE(kwnames))) {
        // 3 = names/types/values
        // 2 = names/values
        // final +1 is kwd_mark
        key_size += (typed ? kw_count * 3 : kw_count * 2) + 1;
    }
    return key_size;
}

static void
fill_key_buffer(PyObject **args,
                Py_ssize_t nargs,
                PyObject *kwnames,
                int typed,
                PyObject **out,
                Py_ssize_t out_size)
{
    ...
    // copy args
    memcpy(out, args, nargs * sizeof(PyObject *));
    Py_ssize_t key_pos = nargs;
    PyObject *const *kwargs = args + nargs;
    ...
    if (kw_count) {
        ...
        for (Py_ssize_t i = 0; i < kw_count; ++i) {
            ...
            out[key_pos++] = kwargs[i];
        }
    }
    if (typed) {
        for (Py_ssize_t i = 0; i < nargs; ++i) {
            out[key_pos++] = (PyObject *)Py_TYPE(args[i]);
        }
        if (kw_count) {
            for (Py_ssize_t i = 0; i < kw_count; ++i) {
                out[key_pos++] = (PyObject *)Py_TYPE(kwargs[i]);
            }
        }
    }
...
static PyObject *
infinite_lru_cache_wrapper(lru_cache_object *self,
                           PyObject **args,
                           ssize_t nargsf,
                           PyObject *kwnames)
{
    ...
    Py_ssize_t key_size = compute_key_size(nargs, kwnames, self->typed);
    PyObject **stack;
    if (key_size == nargs) {
        stack = args;
    } else {
        stack = alloca(key_size * sizeof(PyObject *));
        fill_key_buffer(args, nargs, kwnames, self->typed, stack, key_size);
    }
    Py_hash_t hash = _PyTuple_HashStack(stack, key_size);
    if (hash == -1) {
        return NULL;
    }
    result =
        _PyDict_GetItem_StackKnownHash(self->cache, stack, key_size, hash);
    if (result) {
        Py_INCREF(result);
        self->hits++;
        return result;
    ...
    PyObject *key = _PyTuple_FromArray(stack, key_size);
    if (key == NULL) {
        Py_DECREF(result);
        return NULL;
    }
...
static PyObject *bounded_lru_cache_wrapper_miss(lru_cache_object *self,
                                                PyObject *result,
                                                Py_hash_t hash,
                                                PyObject *key);

static inline PyObject *
lru_cache_extract_hit(lru_cache_object *self, lru_list_elem *link)
{
    lru_cache_extract_link(link);
    lru_cache_append_link(self, link);
    PyObject *result = link->result;
    self->hits++;
    Py_INCREF(result);
    return result;
}
...
static PyObject *
bounded_lru_cache_wrapper(lru_cache_object *self,
                          PyObject **args,
                          size_t nargsf,
                          PyObject *kwnames)
{
    ...
    link = (lru_list_elem *)_PyDict_GetItem_StackKnownHash(
        self->cache, stack, key_size, hash);
    if (link != NULL) {
        return lru_cache_extract_hit(self, link);
    }
    if (PyErr_Occurred()) {
        return NULL;
    }
    self->misses++;
    int prev_misses = self->misses;
    result = _PyObject_Vectorcall(self->func, args, nargsf, kwnames);
    if (!result) {
        return NULL;
    }
    if (prev_misses != self->misses) {
        testresult =
            _PyDict_GetItem_StackKnownHash(self->cache, stack, key_size, hash);
        if (testresult != NULL) {
    ...
    return bounded_lru_cache_wrapper_miss(self, result, hash, key);
}

static PyObject *
bounded_lru_cache_wrapper_miss(lru_cache_object *self,
                               PyObject *result,
                               Py_hash_t hash,
                               PyObject *key)
{
...
/*[clinic end generated code: output=df57f5af2f2bea3b input=3775250c03500586]*/
{
    ...
    if (maxsize_O == Py_None) {
        vectorcall = (vectorcallfunc)infinite_lru_cache_wrapper;
        ...
        if (maxsize == 0)
            vectorcall = (vectorcallfunc)uncached_lru_cache_wrapper;
        else
            vectorcall = (vectorcallfunc)bounded_lru_cache_wrapper;
    } else {
...
static PyTypeObject lru_cache_type = {
    ...
    offsetof(lru_cache_object, vectorcall), /* tp_vectorcall_offset */
    0,                                      /* tp_getattr */
    0,                                      /* tp_setattr */
    0,                                      /* tp_as_async */
    0,                                      /* tp_repr */
    ...
    0,                                   /* tp_init */
    0,                                   /* tp_alloc */
    _functools__lru_cache_wrapper,       /* tp_new */
};
