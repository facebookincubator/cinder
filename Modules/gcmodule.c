    ...
    while (gc != containers) {
        ...
        */
        if (_Py_IsImmortal(FROM_GC(gc))) {
            gc_list_move(gc, &state->permanent_generation.head);
            gc = next;
            continue;
        }
...
clear_freelists(void)
{
    ...
    (void)_PyCell_ClearFreeList();
    (void)_PyFunction_ClearFreeList();
    (void)_PyGen_ClearFreeList();
    (void)_PyWrapper_ClearFreeList();
}
...
/* facebook begin */
PyDoc_STRVAR(gc_get_frozen_objects__doc__,
"get_frozen_objects() -> list\n\
Get objects in the permanent generation\n");

#define GC_GET_FROZEN_OBJECTS_METHODDEF    \
    {"get_frozen_objects", (PyCFunction)gc_get_frozen_objects, METH_NOARGS, \
     gc_get_frozen_objects__doc__},

static PyObject *
gc_get_frozen_objects(PyObject *module, PyObject *Py_UNUSED(ignored))
{
    PyGC_Head *gc, *list;
    PyObject *result;
    ...
    for (gc = GC_NEXT(list); gc != list; gc = GC_NEXT(gc)) {
        if (PyList_Append(result, FROM_GC(gc)) < 0) {
            Py_DECREF(result);
            return NULL;
        }
    }
...
PyDoc_STRVAR(gc_get_all_referrers__doc__,
"get_all_referrers(*objs) -> list\n\
Return the list of objects (including any in the permanent generation) \
that directly refer to any of objs.");

#define GC_GET_ALL_REFERRERS_METHODDEF    \
    {"get_all_referrers", (PyCFunction)gc_get_all_referrers, \
    METH_VARARGS, gc_get_all_referrers__doc__},

static PyObject *
gc_get_all_referrers(PyObject *self, PyObject *args)
{
    int i;
    PyGC_Head *permanent_gen_head;
    PyObject *result;

    result = PyList_New(0);
    if (!result) return NULL;

    // Walk permanent generation first
    permanent_gen_head = &_PyRuntime.gc.permanent_generation.head;
    if (!(gc_referrers_for(args, permanent_gen_head, result))) {
        Py_DECREF(result);
        return NULL;
    }
...
static PyMethodDef GcMethods[] = {
    ...
    GC_GET_FROZEN_OBJECTS_METHODDEF
    GC_GET_ALL_REFERRERS_METHODDEF
    {NULL,      NULL}           /* Sentinel */
};
