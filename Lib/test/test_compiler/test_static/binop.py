...
class BinopTests(StaticTestBase):
    ...
    def test_mixed_binop_sign(self):
        ...
        """
        with self.assertRaisesRegex(
            TypedSyntaxError, "can't compute pow of uint8 and int8"
        ):
        ...
        """
        with self.assertRaisesRegex(
            TypedSyntaxError, "can't compute pow of int8 and uint8"
        ):
            self.compile(codestr)
        codestr = """
        ...
        """
        with self.assertRaisesRegex(
            TypedSyntaxError, "can't compute pow of double and uint8"
        ):
            self.compile(codestr)
