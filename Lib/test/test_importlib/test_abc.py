...
class SourceLoaderBytecodeTests(SourceLoaderTestHarness):
    ...
    def verify_code(self, code_object, *, bytecode_written=False):
        ...
        if bytecode_written:
            ...
            # marshal.dumps internally behaves differently
            # depending on refcount of value being serialized:
            # non-shared (rc=1) values are written as is and
            # shared (rc>1) are persisted as reference
            # JIT compiled version of this function can have different
            # refcount for code_object value (interpreter will have at least
            # refs from current and parent frames vs JIT that uses borrowed references).
            # Add extra reference to code_object to make serialized representation more stable
            self._co = code_object
            data.extend(marshal.dumps(code_object))
            self._co = None
