    ...
    def test_write_rewind_write(self):
        ...
        for i in range(0, len(b)):
            for j in range(i, len(b)):
                ...
                # When running under the JIT we will eagerly reclaim references
                # after their last use. Without the following line, bufio will
                # be reclaimed after the call to flush() and will close the
                # underlying io object (raw).
                self.assertIs(bufio, bufio)
    ...
    def check_daemon_threads_shutdown_deadlock(self, stream_name):
        ...
        if res.rc != 0:
            ...
            pattern = (r"Fatal Python error: could not acquire lock "
                       rf"for <(_io\.)?BufferedWriter name='<{stream_name}>'> "
