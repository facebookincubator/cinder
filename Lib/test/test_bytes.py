...
class ByteArrayTest(BaseBytesTest, unittest.TestCase):
    ...
    def test_resize_forbidden(self):
        ...
        # TODO(T62694321) keep v alive in the JIT
        self.assertIs(v, v)
...
class BytearrayPEP3137Test(unittest.TestCase):
    ...
    def test_returns_new_copy(self):
        ...
        newval = val.split()[0]
        self.assertEqual(val, newval)
        self.assertIsNot(val, newval, 'returned val on a mutable object')

        newval = val.rsplit()[0]
        self.assertEqual(val, newval)
        self.assertIsNot(val, newval, 'returned val on a mutable object')

        newval = val.partition(b".")[0]
        self.assertEqual(val, newval)
        self.assertIsNot(val, newval, 'returned val on a mutable object')

        newval = val.rpartition(b".")[2]
        self.assertEqual(val, newval)
        self.assertIsNot(val, newval, 'returned val on a mutable object')

        newval = val.splitlines()[0]
        self.assertEqual(val, newval)
        self.assertIsNot(val, newval, 'returned val on a mutable object')

        newval = val.replace(b"", b"")
        self.assertEqual(val, newval)
        self.assertIsNot(val, newval, 'returned val on a mutable object')
