...
<paragraph id="A-0001" lvl="0">A new cultivar of Begonia plant named &lsquo;BCT9801BEG&rsquo;.</paragraph>
    ...
    # This test can fail if the method cache is not initialized in a specific
    # order. When CPython starts up, it fills the global method cache with
    # references to None. Each time a previously unoccupied entry is filled,
    # the reference it held to None is destroyed (thus decrementing the
    # refcount for None). Now, consider the code below. The refcount of None
    # will be decremented if any of the method lookups that occur after the
    # first `sys.getrefcount(None)` result in an empty method cache entry being
    # filled.  As a result, the second `sys.getrefcount(None)` will return a
    # value less than the first call and the test will fail.
    @unittest.skipIf(sys.gettrace(), "Skips under coverage.")
    @unittest.skipUnderCinderJIT("depends on method cache initialization order")
