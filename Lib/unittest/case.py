...
def skipIfDebug(reason):
    """
    Skip a test if python was built `--with-pydebug`
    """
...
def skipUnderUwsgi():
    """
    Skip tests when running as uwsgi binary (for BOLT collection).
    """
    if sys.executable.endswith("uwsgi"):
        return skip("not supported under uwsgi")
    return _id

def skipIfASANEnabled(reason):
