...
class VerifierOpArgTests(VerifierTests):
    ...
    def test_FUNC_CREDENTIAL_with_invalid_oparg_index_raises_exception(self):
        code = self.compile_helper(
            "",
            bytecodes=[(op.opcode.LOAD_CONST, 0), (op.opcode.FUNC_CREDENTIAL, 1)],
            consts=(3,),
            stacksize=2,
        )
        self.assertRaisesRegex(
            VerificationError,
            "Argument index 1 out of bounds for size 1 for operation FUNC_CREDENTIAL @ offset 2",
            Verifier.validate_code,
            code,
        )
    ...
    def test_READONLY_OPERATION_with_invalid_oparg_index_raises_exception(self):
        code = self.compile_helper(
            "",
            bytecodes=[(op.opcode.LOAD_CONST, 0), (op.opcode.READONLY_OPERATION, 1)],
            consts=(3,),
            stacksize=2,
        )
        self.assertRaisesRegex(
            VerificationError,
            "Argument index 1 out of bounds for size 1 for operation READONLY_OPERATION @ offset 2",
            Verifier.validate_code,
            code,
        )
