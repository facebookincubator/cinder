  ...
  while (!queue.empty()) {
    ...
    for (auto bc_it = bc_block.begin(); bc_it != bc_block.end(); ++bc_it) {
      ...
      switch (bc_instr.opcode()) {
        ...
        default: {
          // NOTREACHED
          JIT_ABORT("unhandled opcode: %d", bc_instr.opcode());
          break;
        }
      }
...
static inline BinaryOpKind get_bin_op_kind(
  ...
  switch (bc_instr.opcode()) {
    ...
    default: {
      JIT_ABORT("unhandled binary op %d", bc_instr.opcode());
      // NOTREACHED
      break;
    }
  }
...
static inline InPlaceOpKind get_inplace_op_kind(
  ...
  switch (bc_instr.opcode()) {
    ...
    default: {
      JIT_ABORT("unhandled inplace op %d", bc_instr.opcode());
      // NOTREACHED
      break;
    }
  }
...
static inline UnaryOpKind get_unary_op_kind(
  ...
  switch (bc_instr.opcode()) {
    ...
    default:
      JIT_ABORT("unhandled unary op %d", bc_instr.opcode());
      // NOTREACHED
      break;
  }
}
...
static inline BinaryOpKind get_primitive_bin_op_kind(
  ...
  switch (bc_instr.oparg()) {
    ...
    default: {
      JIT_ABORT("unhandled binary op %d", bc_instr.oparg());
      // NOTREACHED
      break;
    }
  }
...
static inline bool is_double_binop(int oparg) {
  switch (oparg) {
    ...
    default: {
      JIT_ABORT("Invalid binary op %d", oparg);
      // NOTREACHED
      break;
    }
  }
...
static inline Type element_type_from_seq_type(int seq_type) {
  switch (seq_type) {
    ...
    case SEQ_ARRAY_INT64:
      return TCInt64;
    default:
      JIT_ABORT("invalid sequence type: (%d)", seq_type);
      // NOTREACHED
      break;
  }
}
...
void HIRBuilder::emitSequenceGet(
  ...
  if (oparg == SEQ_LIST || oparg == SEQ_LIST_INEXACT ||
    ...
    JIT_ABORT("Unsupported oparg for SEQUENCE_GET: %d", oparg);
  }
...
void HIRBuilder::emitSequenceSet(
  ...
  if (oparg == SEQ_ARRAY_INT64) {
    ...
    JIT_ABORT("Unsupported oparg for SEQUENCE_SET: %d", oparg);
  }
...
void HIRBuilder::emitRaiseVarargs(
  ...
  switch (bc_instr.oparg()) {
    ...
    case 0:
      ...
      break;
    default:
      JIT_ABORT("unsupported RAISE_VARARGS op: %d", bc_instr.oparg());
      break;
  }
}
