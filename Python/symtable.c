...
struct symtable *
_PySymtable_BuildObjectOptFlags(mod_ty mod,
                                 PyObject *filename,
...
{
    return _PySymtable_BuildObjectOptFlags(mod, filename, future, 0);
}
...
local_names_include_any_ste_defs(PySTEntryObject *comp_entry, PyObject *local_names)
{
    ...
    while (PyDict_Next(comp_entry->ste_symbols, &pos, &k, &v)) {
        ...
        if (val & DEF_BOUND) {
            int ok = PySet_Contains(local_names, k);
            if (ok < 0) {
                return -1;
            }
            if (ok) {
...
is_free_in_comp_children(PySTEntryObject *comp, PyObject *key)
{
    for (Py_ssize_t i = 0; i < PyList_GET_SIZE(comp->ste_children); ++i) {
        ...
        long scope = PyST_GetScope(child_ste, key);
        if (scope == FREE) {
            return 1;
        }
    }
