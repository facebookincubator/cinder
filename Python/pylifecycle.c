...
pycore_init_types(void)
{
    ...
    if (_PyFunction_InitSwitchboard() < 0) {
        Py_FatalError("Py_Initialize: can't init function switchboard");
    }

    if (_PyType_InitSwitchboard() < 0) {
        Py_FatalError("Py_Initialize: can't init type switchboard");
    }
...
static PyStatus
pycore_init_array(PyInterpreterState *interp)
{
    PyObject *arraymod = _PyArray_Init();
    if (arraymod == NULL) {
        return _PyStatus_ERR("can't initialize array module");
    }
    _PyImport_FixupBuiltin(arraymod, "array", interp->modules);
    return _PyStatus_OK();
}
              ...
              const PyConfig *config)
{
    ...
    *interp_p = interp;
    ...
    status = pycore_init_array(interp);
    if (_PyStatus_EXCEPTION(status)) {
        return status;
    }
...
static int
_Py_FinalizeEx(int skip_cleanup);
...
Py_Exit(int sts)
{
    if (_Py_FinalizeEx(_Py_SkipFinalCleanup) < 0) {
        sts = 120;
    }
