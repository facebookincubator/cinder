...
PyTuple_New(Py_ssize_t size) {
    PyObject *t = _PyTuple_NewNoTrack(size);
    if (t && size != 0) {
...
Py_hash_t
_PyTuple_HashStack(PyObject *const *item, Py_ssize_t len)
{
    Py_ssize_t i;
...
{
    return _PyTuple_HashStack(v->ob_item, Py_SIZE(v));
}
...
PyObject *
_PyTuple_FromArrayNoTrack(PyObject *const *src, Py_ssize_t n)
{
    PyTupleObject *tuple = (PyTupleObject *)_PyTuple_NewNoTrack(n);
    if (tuple == NULL) {
        return NULL;
    }
    PyObject **dst = tuple->ob_item;
...
PyObject *
_PyTuple_Repeat(PyTupleObject *a, Py_ssize_t n)
{
