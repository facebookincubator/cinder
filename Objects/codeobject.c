...
void
_PyCode_DebugDump(PyCodeObject *code)
{
#define DUMP_FIELD(__x)                                                       \
    fprintf(stderr, "%-16s = 0x%llx\n", #__x, (unsigned long long)(code->__x))
    ...
    DUMP_FIELD(co_code);
    DUMP_FIELD(co_consts);
    DUMP_FIELD(co_names);
    DUMP_FIELD(co_varnames);
    DUMP_FIELD(co_freevars);
    DUMP_FIELD(co_cellvars);
    DUMP_FIELD(co_cell2arg);
    DUMP_FIELD(co_filename);
    fprintf(stderr, "co_filename = %s\n", PyUnicode_AsUTF8(code->co_filename));
    DUMP_FIELD(co_name);
    fprintf(stderr, "co_name = %s\n", PyUnicode_AsUTF8(code->co_name));
    DUMP_FIELD(co_lnotab);
    DUMP_FIELD(co_zombieframe);
    DUMP_FIELD(co_weakreflist);
    DUMP_FIELD(co_extra);
    DUMP_FIELD(co_rawcode);
    DUMP_FIELD(co_codelen);

    fprintf(stderr, "Dumping raw code:\n");
    for (unsigned int i = 0; i < code->co_codelen / sizeof(_Py_CODEUNIT);
         i++) {
        if (i % 4 == 0) {
            fprintf(stderr, "%8d: ", i);
        }
        int val = code->co_rawcode[i];
        fprintf(stderr, "%-16.4x", val);
        if (i % 4 == 3) {
            fprintf(stderr, "\n");
        }
    }
    ...
    struct _PyShadowCode *shadow = code->co_cache.shadow;
    if (shadow == NULL) {
        fprintf(stderr, "No shadow code.\n");
        return;
    }
    fprintf(stderr, "Dumping shadow code:\n");
    for (unsigned int i = 0; i < shadow->len / sizeof(_Py_CODEUNIT); i++) {
        if (i % 4 == 0) {
            fprintf(stderr, "%8d: ", i);
        }
        int val = shadow->code[i];
        fprintf(stderr, "%-16.4x", val);
        if (i % 4 == 3) {
            fprintf(stderr, "\n");
        }
