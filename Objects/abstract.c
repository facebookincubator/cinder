...
PyObject_Format(PyObject *obj, PyObject *format_spec)
{
    ...
    /* And call it. */
    result = _PyObject_Call1Arg(meth, format_spec);
    Py_DECREF(meth);
...
Py_ssize_t
_PySequence_CheckBounds(PyObject *s, Py_ssize_t i)
{
    i = i < 0 ? i + Py_SIZE(s) : i;
    if (i < 0 || i >= Py_SIZE(s)) {
...
PyObject_IsInstance(PyObject *inst, PyObject *cls)
{
    ...
    if (checker != NULL) {
        ...
        res = _PyObject_Call1Arg(checker, inst);
        Py_LeaveRecursiveCall();
        Py_DECREF(checker);
        if (res != NULL) {
...
PyObject_IsSubclass(PyObject *derived, PyObject *cls)
{
    ...
    if (checker != NULL) {
        ...
        res = _PyObject_Call1Arg(checker, derived);
        Py_LeaveRecursiveCall();
        Py_DECREF(checker);
        if (res != NULL) {
            ok = PyObject_IsTrue(res);
...
PyIter_Send(void *tstate, PyObject *iter, PyObject *arg, PyObject **result)
{
    ...
    if (PyType_HasFeature(Py_TYPE(iter), Py_TPFLAGS_HAVE_AM_SEND)) {
        ...
        PySendResult res = Py_TYPE(iter)->tp_as_async->am_send(tstate, iter, arg, result);
        return res;
    }
