...
_PyObject_LookupAttr(PyObject *v, PyObject *name, PyObject **result)
{
    ...
    if (tp->tp_getattro == PyType_Type.tp_getattro) {
        *result = _PyType_LookupAttr(v, name);
        if (*result != NULL) {
            return 1;
        }
        if (PyErr_Occurred()) {
    ...
    if (tp->tp_getattro == PyModule_Type.tp_getattro) {
        *result = _PyModule_LookupAttr(v, name);
        if (*result != NULL) {
            return 1;
        }
        if (PyErr_Occurred()) {
                                 ...
                                 PyObject *dict, int suppress)
{
    ...
    */
    ...
    if (dict != NULL) {
        Py_INCREF(dict);
        res = _PyDict_GetAttrItem_Unicode(dict, name);
        if (res != NULL) {
            Py_INCREF(res);
            Py_DECREF(dict);
            goto done;
...
PyObject *
PyObject_GenericGetAttrString(PyObject *obj, char *name)
{
    PyObject *res = NULL;
    ...
    if (!_bytes_isascii(name, len)) {
        ...
        res = _PyObject_GenericGetAttrWithDict(obj, name_obj, NULL, 0);
        Py_DECREF(name_obj);
        return res;
    }
    ...
    descr = _PyType_LookupString(tp, name, len, hash);
...
PyObject _Py_NoneStruct = {
  ...
  kImmortalInitialCount, &_PyNone_Type
};
...
#define INIT_TYPE(TYPE, NAME) \
    ...
    INIT_TYPE(&PyWaitHandle_Type, "wait handle");
