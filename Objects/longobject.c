        ...
        if (rem)
            ...
            while (shift_digits > 0)
                if (a->ob_digit[--shift_digits]) {
                    /* The assertion below guarantees that v_rshift
                     * initializes x_digits[0] */
...
static PyObject *
vectorcall_long(PyTypeObject *type, PyObject **args, size_t nargsf, PyObject *kwnames)
{
    ...
    fastargs = _PyArg_UnpackKeywords(args, nargs, NULL, kwnames, &_parser, 0, 2, 0, argsbuf);
    if (!fastargs) {
        goto exit;
    }
    if (nargs < 1) {
...
exit:
    ...
    return long_new_impl(type, x, obase);
}
...
PyTypeObject PyLong_Type = {
    ...
    .tp_vectorcall = (vectorcallfunc)vectorcall_long
};
