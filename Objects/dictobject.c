...
/* Global counter used to set ma_version_tag field of dictionary.  It is
 * incremented each time that a dictionary is created and each time that a
...
#define IS_CHECKED_DICT(x)                                                    \
    (_PyClassLoader_GetGenericTypeDef((PyObject *)x) == &_PyCheckedDict_Type)

static inline int
_PyDict_CheckIncludingChecked(PyObject *x)
{
    return PyDict_Check(x) || IS_CHECKED_DICT(x);
}

int
_PyCheckedDict_Check(PyObject *x)
{
...
#define CHECK(expr) \
    ...
    assert(op != NULL);
    CHECK(_PyDict_CheckIncludingChecked(op));
...
found:
    ...
    if (new_value || (getting && PyDeferred_CheckExact(value))) {
        ...
        if (ep->me_value == new_value) {
            ...
            dict_modify_key(mp, key, new_value);
        }
        value = new_value;
    }
    *value_addr = value;
                 ...
                 Py_hash_t hash, PyObject **value_addr, int getting)
{
    ...
    /* Make sure this function doesn't have to handle non-unicode keys,
       ...
       that here. */
    if (!PyUnicode_CheckExact(key)) {
        dict_set_lookup(mp, lookdict_deferred);
...
_PyDict_HasOnlyStringKeys(PyObject *dict)
{
    Py_ssize_t pos = 0;
    PyObject *key, *value;
    assert(_PyDict_CheckIncludingChecked(dict));
    /* Shortcut */
...
dictresize(PyDictObject *mp, Py_ssize_t minsize)
{
    ...
    if (oldkeys->dk_lookup == lookdict ||
        ...
        dict_set_lookup(mp, oldkeys->dk_lookup);
...
PyDict_GetItemWithError(PyObject *op, PyObject *key)
{
    ...
    if (!_PyDict_CheckIncludingChecked(op)) {
        PyErr_BadInternalCall();
        return NULL;
    }
    if (!PyUnicode_CheckExact(key) ||
...
_PyDict_GetItemIdWithError(PyObject *dp, struct _Py_Identifier *key)
{
    ...
    if (kv == NULL)
        return NULL;
    return _PyDict_GetItem_UnicodeExact(dp, kv);
}
          ...
          PyObject **pvalue, Py_hash_t *phash, int resolve)
{
    ...
    if (resolve && UNLIKELY(DICT_HAS_DEFERRED(mp))) {
...
dict_subscript(PyDictObject *mp, PyObject *key)
{
    ...
    if (ix == DKIX_EMPTY || value == NULL) {
        if (!PyDict_CheckExact(mp)) {
            ...
            if (missing != NULL) {
                res = _PyObject_Call1Arg(missing, key);
                Py_DECREF(missing);
                return res;
            }
...
static int
dict_update_common_fast(PyObject *self, PyObject *arg, PyObject *kwds) {
    int result = 0;
                   ...
                   const char *methname)
{
    ...
    return dict_update_common_fast(self, arg, kwds);
}
...
/*[clinic end generated code: output=bba707729dee05bf input=279ddb5790b6b107]*/
{
    ...
    assert(!IS_CHECKED_DICT(self) ||
           _PyClassLoader_CheckParamType((PyObject *)self, key, 0));
    if (!PyUnicode_CheckExact(key) ||
        (hash = ((PyASCIIObject *) key)->hash) == -1) {
        hash = PyObject_Hash(key);
        if (hash == -1)
...
PyDict_SetDefault(PyObject *d, PyObject *key, PyObject *defaultobj)
{
    ...
    if (!_PyDict_CheckIncludingChecked(d)) {
        PyErr_BadInternalCall();
        return NULL;
    }
    assert(!IS_CHECKED_DICT(d) || _PyClassLoader_CheckParamType(d, key, 0));
    assert(!IS_CHECKED_DICT(d) ||
           _PyClassLoader_CheckParamType(d, defaultobj, 1));
...
/*[clinic end generated code: output=e65fcb04420d230d input=1c38a49f21f64941]*/
{
    ...
    self->ma_used--;
    dict_modify_key(self, old_key, old_key);
    ASSERT_CONSISTENT(self);

    if (UNLIKELY(DICT_HAS_DEFERRED(self))
...
dictiter_iternextkey(dictiterobject *di)
{
    ...
    if (d == NULL)
        return NULL;
    assert(_PyDict_CheckIncludingChecked((PyObject *)d));
...
dictiter_iternextvalue(dictiterobject *di)
{
    ...
    if (UNLIKELY(DICT_HAS_DEFERRED(d))
        ...
        if (*value_ptr != new_value) {
            ...
            Py_DECREF(*value_ptr);
            *value_ptr = new_value;
            dict_modify_key(d, key, new_value);
        }
        Py_INCREF(new_value);
        Py_DECREF(key);
        Py_DECREF(value);
...
{
    assert(_PyDict_CheckIncludingChecked((PyObject *)self));
    ...
    if ((tp->tp_flags & Py_TPFLAGS_HEAPTYPE) && (cached = CACHED_KEYS(tp))) {
        ...
        if (dict == NULL) {
            if (tp->tp_flags & Py_TPFLAGS_WARN_ON_SETATTR &&
                _PyErr_RaiseCinderWarning("WARN001: Dictionary created for flagged instance", (PyObject *) tp, key)) {
                return -1;
            }
                                 ...
                                 Py_hash_t hash)
{
    ...
    if (mp->ma_keys->dk_lookup == lookdict ||
        ...
        Py_DECREF(name_obj);
    } else if (mp->ma_keys->dk_lookup == lookdict_unicode_deferred) {
        assert(mp->ma_values == NULL);
top:
        dk = mp->ma_keys;
        ep0 = DK_ENTRIES(dk);
                ...
                if (ep->me_hash == hash &&
                    ...
                    if (new_value || PyDeferred_CheckExact(value)) {
                        ...
                        if (ep->me_value == new_value) {
                            ...
                            PyObject *name_obj = PyUnicode_FromString(key);
                            if (name_obj != NULL) {
                                dict_modify_key(mp, name_obj, new_value);
                                Py_DECREF(name_obj);
                            }
                        }
                        value = new_value;
...
/* Same as _PyDict_GetItem_Unicode, but it resolves any deferred objects
 * only if the deferred object is not resolving to itself, otherwise it
 * returns as missing item. */
PyObject *
_PyDict_GetAttrItem_Unicode(PyObject *op, PyObject *key)
{
    Py_hash_t hash;
    Py_ssize_t ix;
    PyDictKeysObject *dk;
...
chkdict_update_common_fast(PyObject *self, PyObject *arg, PyObject *kwds)
{
    ...
    if (arg != NULL) {
        ...
        } else if (_PyDict_CheckIncludingChecked(arg)) {
...
const _Py_SigElement *const chkdict_sig[] = {
...
_PyTypedMethodDef chkdict_getitem_def = {
    dict_subscript, getitem_sig, _Py_SIG_TYPE_PARAM_OPT(1)};

const _Py_SigElement *const setitem_sig[] = {&_Py_Sig_T0, &_Py_Sig_T1, NULL};
_PyTypedMethodDef chkdict_setitem_def = {
    chkdict_ass_sub_unchecked, setitem_sig, _Py_SIG_ERROR};
...
static PyMethodDef chkmapp_methods[] = {
    ...
    {"__sizeof__",
     ...
     METH_NOARGS,
     sizeof__doc__},
    {"get", (PyCFunction)&chkdict_get_def, METH_TYPED, dict_get__doc__},
    {"setdefault",
...
chkdict_richcompare(PyObject *v, PyObject *w, int op)
{
    ...
    if (!_PyDict_CheckIncludingChecked(v) ||
        !_PyDict_CheckIncludingChecked(w)) {
        res = Py_NotImplemented;
    } else if (op == Py_EQ || op == Py_NE) {
        cmp = dict_equal((PyDictObject *)v, (PyDictObject *)w);
        if (cmp < 0)
...
PyObject *
_PyCheckedDict_New(PyTypeObject *type)
{
    dictkeys_incref(Py_EMPTY_KEYS);
...
PyObject *
_PyCheckedDict_NewPresized(PyTypeObject *type, Py_ssize_t minused)
{
    const Py_ssize_t max_presize = 128 * 1024;
    Py_ssize_t newsize;
    PyDictKeysObject *new_keys;
