...
{
    unionobject *alias = (unionobject *)self;
    PyObject *fs = PyFrozenSet_New(alias->args);
    if (fs == NULL) {
        return -1;
    }
    PyObject *to_hash = PyTuple_New(2);
    if (to_hash == NULL) {
        Py_DECREF(fs);
        return -1;
    }

    PyObject *union_type = (PyObject*)&_Py_UnionType;
    Py_INCREF(union_type);
    PyTuple_SET_ITEM(to_hash, 0, union_type);
    PyTuple_SET_ITEM(to_hash, 1, fs);

    Py_hash_t h1 = PyObject_Hash(to_hash);
    Py_DECREF(to_hash);
    if (h1 == -1) {
        return -1;
    }
...
union_subclasscheck(PyObject *self, PyObject *instance)
{
    ...
    for (Py_ssize_t iarg = 0; iarg < nargs; iarg++) {
        PyObject *arg = PyTuple_GET_ITEM(alias->args, iarg);
        if (PyType_Check(arg) &&
            (PyType_IsSubtype((PyTypeObject *)instance, (PyTypeObject *)arg) !=
             0)) {
            Py_RETURN_TRUE;
        }
    }
...
is_typing_module(PyObject *obj)
{
    ...
    int is_typing = PyUnicode_Check(module) &&
...
// Needed to differentiate between typing.Union and other specialforms.
static int
is_typing_union(PyObject *obj)
{
    int has_origin = PyObject_HasAttrString(obj, "__origin__");
    if (has_origin) {
        PyObject *origin = PyObject_GetAttrString(obj, "__origin__");
        if (origin && PyObject_HasAttrString(origin, "_name")) {
            PyObject *name = PyObject_GetAttrString(origin, "_name");
            int is_union = PyUnicode_Check(name) &&
...
union_richcompare(PyObject *a, PyObject *b, int op)
{
    ...
    int _is_typing_union = is_typing_name(b, "_GenericAlias") && is_typing_union(b);
    if (_is_typing_union < 0) {
        goto exit;
    }
    if (_is_typing_union) {
        ...
        if (!PyTuple_CheckExact(b_args)) {
            ...
            PyErr_SetString(
                PyExc_TypeError,
                "__args__ argument of typing.Union object is not a tuple");
            goto exit;
        }
...
flatten_args(PyObject *args)
{
    ...
    for (Py_ssize_t i = 0; i < arg_length; i++) {
        ...
        } else if (arg_type == &_PyNone_Type) {
            PyObject *none_type = (PyObject *)&_PyNone_Type;
            Py_INCREF(none_type);
            PyTuple_SET_ITEM(flattened_args, pos, none_type);
            pos++;
        } else {
            Py_INCREF(arg);
...
is_unionable(PyObject *obj)
{
    ...
    PyTypeObject *type = Py_TYPE(obj);
    return (is_typevar(obj) || is_new_type(obj) || is_special_form(obj) ||
            PyType_Check(obj) ||
            type == &_Py_UnionType);
}
...
union_repr(PyObject *self)
{
    ...
    for (Py_ssize_t i = 0; i < len; i++) {
        if (i > 0 &&
            _PyUnicodeWriter_WriteASCIIString(&writer, " | ", 3) < 0) {
            goto error;
        }
...
static PyMemberDef union_members[] = {
    {"__args__", T_OBJECT, offsetof(unionobject, args), READONLY}, {0}};
...
PyTypeObject _Py_UnionType = {
    PyVarObject_HEAD_INIT(&PyType_Type, 0).tp_name = "types.Union",
