...
static size_t numfree = 0;
static size_t method_maxfreesize = 256;

#define METHODFREELIST_STATS 1

#if METHODFREELIST_STATS
static size_t method_free_list_hits = 0;
static size_t method_free_list_misses = 0;
#endif
...
size_t
_PyMethod_GetFreeListSize(void)
{
    return method_maxfreesize;
}

void
_PyMethod_SetFreeListSize(size_t size)
{
    method_maxfreesize = size;
    while (numfree > method_maxfreesize) {
        PyMethodObject *cur = free_list;
        free_list = (PyMethodObject *)(cur->im_self);
        PyObject_GC_Del(cur);
        numfree--;
    }
}
...
PyObject *
_PyMethod_GetFreeListStats(void)
{
    ...
    PyObject *hits, *misses, *size;
    size = PyLong_FromSize_t(numfree);
#if METHODFREELIST_STATS
    hits = PyLong_FromSize_t(method_free_list_hits);
    misses = PyLong_FromSize_t(method_free_list_misses);
#else
    hits = PyLong_FromLong(-1);
    misses = PyLong_FromLong(-1);
#endif

    if (size != NULL && hits != NULL && misses != NULL) {
        PyTuple_SET_ITEM(tuple, 0, size);
        PyTuple_SET_ITEM(tuple, 1, hits);
        PyTuple_SET_ITEM(tuple, 2, misses);
        return tuple;
    }
